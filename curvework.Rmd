...
for (i in 1:length(sites)){
  ...
  mod=AdjustedData%>%
    ...
    geom_vline(xintercept = StartFeces)+
    geom_vline(xintercept = EndFeces)+
    ylab("log of N1 gene normalized Pct_BCoV, PMMoV")+
    scale_x_date(limits = c(StartFeces,EndFeces))+
    ylim(minSew,maxSew)
...
FitedData=TempData%>%
  ...
  mutate(TotalUnExplaned=roll-mean(roll,na.rm=T))
...
```{r}
FitedData%>%
  filter(!is.na(resid),!is.na(TotalUnExplaned))%>%
  summarize(R2=1-(sum(resid^2)/sum(TotalUnExplaned^2)))
```

R^2 for the integrated model

```{r}
FitedData%>%
  filter(!is.na(residFin))%>%
  summarize(R20=1-(sum(residFin^2)/sum(TotalUnExplaned^2)))
```
